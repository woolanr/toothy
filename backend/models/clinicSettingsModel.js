...
const ClinicSettings = {
    ...
    updateSetting: async (key, value, dataType = 'string') => {
        ...
        `;
        try {
            const [result] = await db.execute(sql, [key, value, dataType, value, dataType]);
            console.log(`clinicSettingsModel: Setting '${key}' updated/inserted. Affected rows: ${result.affectedRows}`);
            return result;
        } catch (error) {
            console.error(`clinicSettingsModel: Error updating setting '${key}':`, error);
            throw error;
        }
    },
    ...
    getSettingByKey: async (key) => {
        ...
        try {
            ...
            console.error(`clinicSettingsModel: Error getting setting by key '${key}':`, error);
            throw error;
        }
    }
